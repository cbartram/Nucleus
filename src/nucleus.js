...
exec(`cd ${config.out} && mkdir ${config.name} && cd ./${config.name}`, (err) => {
  ...
  if (config.style) {
    ...
    try {
      ...
      error(styleError, config, '[Nucleus] Could not create file! Use --dev to view error information');
      return;
    }
  } else {
    ...
    if (config.templatePath !== null) {
      try {
        ...
        if (config.dev) {
          console.log(chalk.blueBright(`[Nucleus Dev] ${templateError}`));
        }
        process.exit(0);
      }
    } else {
      ...
      if (config.functional) {
        try {
          ...
          error(functionalError, config, '[Nucleus] Could not create functional component! Use --dev to view error information');
          process.exit(0);
        }
      } else {
        try {
          ...
          error(defaultError, config, '[Nucleus] Could not create default component! Use --dev to view error information');
          process.exit(0);
        }
      }
    }
